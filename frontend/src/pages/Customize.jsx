...
function Customize() {
  ...
  const handleImage=(e)=>{
    ...
    setFrontendImage(URL.createObjectURL(file));//URL is a class and dotcreateobjectURL is a function which convert the file object in url
  }
  return (
    <div className='w-full h-[100vh] overflow-y-auto bg-gradient-to-t from-[black] to-[#181879] flex justify-center items-center flex-col p-[20px]'>
      <MdKeyboardBackspace className='absolute top-[30px] left-[30px] cursor-pointer text-white w-[25px] h-[25px]' onClick={()=>navigate("/")}/> 
      <h1 className='text-white text-[30px] mb-[20px] text-center'>Select your <span className='text-blue-200'>Assistant Image</span></h1>
      <div className='w-full max-w-[900px] flex justify-center items-center flex-wrap gap-[15px]'>
        ...
        <Card image={image6}/>
        <Card image={image7}/>
          <div className={`w-[80px] h-[160px] lg:w-[150px] lg:h-[250px] bg-[#020220] border-2
          border-[#0000ff7c] rounded-2xl overflow-hidden hover:shadow-2xl hover:shadow-blue-950 
          ...
          onClick={()=>{inputImage.current.click()
            ...
            {!frontendImage && <RiImageAddLine className='text-white w-[25px] h-[25px] items-center'/>}
           {frontendImage && <img src={frontendImage} className='h-full object-cover'/>}
          </div>
          <input type="file" accept='image/*'ref={inputImage} hidden onChange={handleImage}/>
      </div>
      {selectedImage && <button className='min-w-[80px] h-[40px] mt-[15px] cursor-pointer bg-white rounded-full text-black font-semibold text-[19px]' onClick={()=>navigate("/customize2")}>Next</button>}
